...
class Main(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        monitor = SteamControllerMonitor(self)
        
        if not SteamController().is_enabled():
            SteamController().enable()
            SteamController().start()            

        while not monitor.abortRequested():
            if monitor.waitForAbort():
                SteamController().stop()
                SteamController().disable()
...
class SteamControllerMonitor(xbmc.Monitor):

    def __init__(self, *args, **kwargs):
        xbmc.Monitor.__init__(self)

    def onSettingsChanged(self):
        SteamController().restart()

if ( __name__ == "__main__" ):
    Main()

    del SteamControllerMonitor
